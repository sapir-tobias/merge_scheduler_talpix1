...
import type { PlacedCourse, SemesterId, Faculty, DayKey, Blocker } from '../types'
...
function buildSlots(choices: ChoiceEntry[]): CourseSlot[] {
...
function countCollisions(choices: ChoiceEntry[], blockers: Blocker[]): number {
  const slots = buildSlots(choices)
  let count = 0
...
function getAllConfigs(placed: PlacedCourse[], blockers: Blocker[]): Config[] {
  ...
  return combos
    .map(choices => ({ choices, collisions: countCollisions(choices, blockers) }))
    .sort((a, b) => a.collisions - b.collisions)
}

function MiniSchedule({ choices, collisions, blockers }: { choices: Config['choices']; collisions: number; blockers: Blocker[] }) {
  const slots = buildSlots(choices)
...
export default function SchedulePreviewPanel({ semesterId, placed, onClose }: Props) {
  ...
  const configs = getAllConfigs(placed, semBlockers)
  const perPage = 3
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    <div className={cn('transition-all duration-150', isHovered && 'blur-[2px] scale-[0.98]')}>
                      <MiniSchedule choices={config.choices} collisions={config.collisions} blockers={semBlockers} />
                    </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] text-stone-400">
                      {config.choices.map(c => {
                        const code = COURSE_MAP.get(c.courseId)?.code ?? ''
