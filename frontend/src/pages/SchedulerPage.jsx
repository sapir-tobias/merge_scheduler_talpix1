...
export default function SchedulerPage() {
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      if (!(e.target).closest('[data-blocker-popover]')) setBlockerOpen(false)
    }
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      if (!(e.target).closest('[data-load-plan-popover]')) setLoadPlanOpen(false)
    }
    setTimeout(() => document.addEventListener('mousedown', handler), 0)
    return () => document.removeEventListener('mousedown', handler)
  }, [loadPlanOpen])
