...
function countCollisionsForOption(course: Course, optionId: string, placed: PlacedCourse[], semesterId: SemesterId, courseMap: CourseMap): number {
  const option = course.lectureOptions.find(o => o.id === optionId)
  if (!option) return 0
  let collisions = 0
  for (const p of placed) {
    if (p.semesterId !== semesterId || p.courseId === course.id) continue
    const pc = courseMap.get(p.courseId)
    if (!pc) continue
...
export function pickBestOption(course: Course, placed: PlacedCourse[], semesterId: SemesterId, courseMap: CourseMap): string {
  ...
  for (const opt of course.lectureOptions) {
    const c = countCollisionsForOption(course, opt.id, placed, semesterId, courseMap)
...
function minExamSeparation(candidate: Course, placed: PlacedCourse[], semesterId: SemesterId, courseMap: CourseMap): number {
  let min = Infinity
  for (const p of placed) {
...
export function filterAndScore(
  ...
  return allCourses
    ...
    .map(c => {
      const cs = scoreCourse(c, placed, exemptions, semesterId, filters, courseMap)
      const nm = fuzzyMatch(searchQuery, c.name)
      const cm = fuzzyMatch(searchQuery, c.code)
