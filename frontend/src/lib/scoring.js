...
function countCollisionsForOption(course, optionId, placed, semesterId, courseMap) {
...
export function pickBestOption(course, placed, semesterId, courseMap) {
...
export function pickFirstRecitationOption(course) {
...
function minExamSeparation(candidate, placed, semesterId, courseMap) {
  let min = Infinity
  for (const p of placed) {
...
export function filterAndScore(
  ...
  return allCourses
    ...
    .sort((a, b) => {
      ...
      const combined = (x) => x.score * 0.7 + x.matchScore * 30
