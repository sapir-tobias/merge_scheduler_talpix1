  ...
  return (
    <div data-testid={`${TEST_IDS.SEMESTER_COURSE_LIST.CHIP}-${placed.courseId}`} className={styles.chip}>
      ...
      <Tooltip text={scoreTooltip} side="top">
        {score.critical
          ? <LuCircleAlert size={10} className={cn(styles.statusIcon, styles.statusCritical)} />
          : score.warning
            ? <LuTriangleAlert size={10} className={cn(styles.statusIcon, styles.statusWarning)} />
            : <LuCircleCheck size={10} className={cn(styles.statusIcon, styles.statusOk)} />
        }
      </Tooltip>
      ...
      {canExpand && (
        ...
        >
          <LuChevronDown size={11} className={cn(styles.chevron, !isExpanded && styles.chevronCollapsed)} />
        </button>
      )}
